...
- **当前阶段**：验收通过 → Lab1 关闭
- **最后更新**：2026-05-11
- **结论**：4 项必做验收标准全 PASS（10 TC / 61 checks），F1-01~F1-15 全部 #VERIFIED
- **遗留**：U-1（PKT_MEM APB 读返回 0，Lab3 解决）；F1-08/F1-12 端到端验证待 Lab3
- **下一步**：进入 Lab2（M3 包处理核心）
- **责任 Agent**：Sign-off Agent → Lab2 DUT Agent
...
Sign-off Agent 按 `acceptance.md` 逐项判定 Lab1 验收标准，独立运行 `make comp` + `make run` 确认结果

### 4.2 独立编译仿真验证

| 命令 | 结果 |
|------|------|
| `make comp` | 0 error, 0 warning（QuestaSim-64 2021.1） |
| `make run` | 61/61 PASS, 0 FAIL（10 TC，seed=1） |

### 4.3 验收标准逐项判定

| # | 验收标准 | 可执行判据 | 判定 | 依据 |
|---|----------|-----------|------|------|
| 1 | APB 基础读写时序正确 | `tc_apb_basic_rw PASS` | **PASS** | TC3: 6/6 checks — RES_PKT_LEN/TYPE/SUM/XOR、STATUS、ERR_FLAG 全部匹配 stub 赋值 |
| 2 | CSR 默认值正确 | `tc_csr_default_rw PASS`，11 寄存器 | **PASS** | TC1: 11/11 checks — CTRL=0, CFG=0xF1, STATUS/IRQ_EN/IRQ_STA/PKT_LEN_EXP/RES_*/ERR_FLAG=0 |
| 3 | PKT_MEM 写入地址映射正确 | `tc_pkt_mem_write PASS`，8 word | **PASS** | TC2: 8/8 checks — SRAM Word[0..7] 与 APB 写入值完全一致 |
| 4 | RES_* 寄存器读通路正确 | CSR 读回值位域正确 | **PASS** | TC3 + TC5: RO 字段来自 M3 stub 信号，写 RO 返回 PSLVERR=1 且值不变 |

### 4.4 Feature-Matrix 交叉验证

F1-01~F1-15 全部 15 个功能点均有对应 TC 覆盖（见 testplan.md 覆盖映射表），运行结果全 PASS。feature-matrix 实现状态和 TB 状态均为 #DONE，验收后统一标记为 #VERIFIED。

### 4.5 非阻塞遗留项

| ID | 描述 | 严重性 | 归属 |
|----|------|--------|------|
| U-1 | PKT_MEM APB 读返回 0 而非 SRAM 数据 | LOW | Lab3 集成时连接 M2 读端口 |
| — | F1-08（W1P start）/ F1-12（busy 写保护）端到端验证 | LOW | Lab3 顶层集成后验证 |

### 4.6 验收结论

**Lab1 验收通过（PASS）**。全部 4 项必做验收标准 PASS，无 FAIL 项，不进入迭代阶段。Lab1 关闭。

---

## 5 迭代阶段

无需迭代。验收阶段全部必做项 PASS，Lab1 直接关闭。
