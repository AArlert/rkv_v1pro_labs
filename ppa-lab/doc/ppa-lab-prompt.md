...
- Lab4 增强目标：`make smoke`、`make regress`、`make cov`
...
2. 审查阶段：检查 RTL 代码设计逻辑是否和 SPEC 文档一致
3. 验证阶段：编写 testcase 并运行仿真，覆盖正常路径、边界条件和错误路径
4. 验收阶段：检查波形和结果，确认 CSR/中断/错误标志 与预期一致
5. 迭代阶段：根据反馈修复问题并完善设计，更新 testplan 与回归集合

# 9 实验日志
在 /labX/doc/log.md 中写入 `#8 验收流程` 规定不同阶段的实验日志
1. 设计阶段：说明设计思想路线上遇到的主要挑战和权衡决策，记录任何对规格的假设或不确定点
2. 审查阶段：说明代码实现与规格的一致性检查过程，记录发现的任何不一致或潜在问题，并说明如何解决或计划解决
3. 验证阶段：说明测试用例设计的覆盖目标和优先级，记录每个用例的输入摘要、预期输出、实际结果和覆盖情况，分析任何失败的原因并说明修复计划
4. 验收阶段：说明验收过程中检查的关键点和结果，记录任何与预期不符的发现，并说明是否需要回到设计或验证阶段进行修复
5. 迭代阶段：总结整个设计-审查-验证-验收的循环过程，记录最终的设计状态、覆盖情况和任何未决风险或未来改进的方向
...
- 所有新增目标（`smoke/regress/cov`）需在 Makefile 中可直接执行并可复现
...
3. 补充统一 Makefile 入口，先实现 `comp/run/rung/clean`，再扩展 `smoke/regress/cov`
