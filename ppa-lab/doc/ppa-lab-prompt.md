...
## 0.2 现状确认
Lab1（M1+M2）和 Lab2（M3）的 RTL 已实现并通过审查。Lab3（顶层集成）和 Lab4（回归覆盖率）待启动。详见 `status.md`
...
- PREADY 固定为 1（spec §3.1）
- APB 写在 ACCESS 阶段生效（spec §3.1）
- 保留/越界地址返回 PSLVERR=1（spec §3.2）
- 写 RO 寄存器返回 PSLVERR=1 且无副作用（spec §3.3）
- CTRL.start 是 W1P，不存储（spec §3.3）
- IRQ_STA 是 RW1C（spec §3.3）
- busy=1 期间写 PKT_MEM 报错且写入无效（spec §3.4）
- done 信号保持至下次合法 start（spec §5.2）
- 三类错误可同时成立（spec §5.4）
...
- **status.md**：当前进度看板
- **feature-matrix.md**：功能清单与状态追踪
- **risk-register.md**：风险与假设登记
- **traceability.md**：spec↔实现↔测试可追溯矩阵

每个实验阶段有独立文档：
- `labX/doc/design-prompt.md` — 本阶段设计指引
- `labX/doc/testplan.md` — 测试计划
- `labX/doc/log.md` — 实验日志（人工维护为主）
