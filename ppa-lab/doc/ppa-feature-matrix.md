...
| ID    | 功能描述                       | Spec § | 实现状态 | TB 状态 | 关联 Testcase | 备注  |
| ----- | -------------------------- | ------ | ---- | ----- | ----------- | --- |
| F3-01 | ppa_top 顶层连线               | §2.1   | #DONE | #DONE  | TC1         | M2 读端口 MUX 仲裁 |
| F3-02 | 端到端链路（APB->CSR->SRAM->FSM->结果） | §2.1   | #DONE | #DONE  | TC1         |     |
| F3-03 | 连续两帧顺序处理                   | §7.2, §10.1 | #DONE | #DONE  | TC2         |     |
| F3-04 | STATUS 总线通路                | §5.2   | #DONE | #DONE  | TC3         |     |
| F3-05 | busy 期间写保护（选做）             | §6.3   | #DONE | #TODO  | —           | RTL 已实现（M1），TB 待补 |
| F3-06 | 中断路径闭环（选做）                 | §8.2   | #DONE | #TODO  | —           | RTL 已实现（M1），TB 待补 |
